<template>
    <ElForm class="register-form" ref="registerRef" :model="registerParam" :rules="registerRules">
        ...
        <ElFormItem prop="username">
            <ElInput placeholder="请输入账号" :prefix-icon="User" v-model="registerParam.user_name" size="large"></ElInput>
        </ElFormItem>
        <ElFormItem prop="password">
            <ElInput placeholder="请输入密码" show-password :prefix-icon="Lock" v-model="registerParam.password" size="large"></ElInput>
        </ElFormItem>
        <ElFormItem prop="email">
            <ElInput placeholder="请输入手机号" :prefix-icon="Message" v-model="registerParam.phone_number" size="large"></ElInput>
        </ElFormItem>
        <ElFormItem>
...
const registerRules:FormRules=reactive({
    user_name:[{required:true,message:"账号不能为空",trigger:'blur'}],
